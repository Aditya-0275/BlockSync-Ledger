...
from Backend.Blockchain.block_module import GENESIS_DATA
...
def test_replace_chain_not_longer(blockchain_three_blocks):
    ...
    with pytest.raises(Exception, match='Can not replace. The incoming chain must be longer.'):
...
def test_replace_chain_bad_chain(blockchain_three_blocks):
    ...
    with pytest.raises(Exception, match='Can not replace. The incoming chain is invalid.'):
        blockchain.replace_chain(blockchain_three_blocks.chain)
